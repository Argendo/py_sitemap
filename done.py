...
def get_links(cur_url, html):
	...
	for a in soup.find_all('a', href=True):
		...
		if not temp_link.startswith(tuple(BAD_PREFS)):
			...
			if temp_link[:len(cur_url)]!=str(cur_url):
	...
	my_gen = (get_links(link, get_html(temp_link)) for link in page_links if not link in GLOBAL_LINKS)
